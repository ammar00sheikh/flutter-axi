 ...
 *
 * Setting `FLUTTER_AXI_SESSION` (or passing `--app <name>` on any command) to
 * a non-default name binds the bridge's port and on-disk state (PID file,
 * snapshot-generation counter, uid->finder refs, app state) to that name, so
 * multiple bridges can run concurrently - one per target app (e.g. "user" and
 * "operator"), agent session, or test worker - without sharing a single bridge
 * or stepping on each other's stale-ref tracking.
 *
 *   flutter-axi --app user launch ...
 *   flutter-axi --app operator launch ...
 *
 * One session owns exactly one `dart mcp-server` child and one target Flutter
 * app - the Dart MCP server has a single "active application" notion, so
 * multi-app orchestration maps to multi-session.
 *
 * Precedence:
 *   port      - FLUTTER_AXI_PORT > deterministic hash of the session name
 *   state dir - always derived from the session name
 *
 * The default session name is "default": port 9424 and `~/.flutter-axi/`.
 */
...
export const DEFAULT_BASE_PORT = 9424;

const SESSION_PORT_RANGE = 1000; // 9425..10424 reserved for named sessions
const STATE_DIR_NAME = ".flutter-axi";

/**
 * Resolve the active session name from `FLUTTER_AXI_SESSION`. Returns
 ...
 * the generation counter, the refs registry, or the bridge itself - can resolve
 * an invalid name into a filesystem path that collapses onto the default
 * session's directory.
 */
export function resolveSessionName(): string {
  const raw = process.env.FLUTTER_AXI_SESSION?.trim();
 ...
 * FLUTTER_AXI_PORT to break a collision.
 */
...
/**
 * Resolve the bridge port for a session: explicit `FLUTTER_AXI_PORT`
 * wins, otherwise the session-derived default.
 */
export function resolveSessionPort(
  name: string = resolveSessionName(),
): number {
  const explicit = process.env.FLUTTER_AXI_PORT;
  if (explicit) {
...
/**
 * State directory for a session. The default session uses `~/.flutter-axi/`;
