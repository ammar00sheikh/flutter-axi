...
flags[3]:
  --app <name>  Target a named session (one session = one app; e.g. --app user,
                --app driver). Defaults to "default".
  --help, -v/-V/--version
...
environment:
  FLUTTER_AXI_SESSION    Session name (same as --app). Each session gets its own
                         ...
                         driven concurrently (e.g. a user app and a driver app).
...
examples:
  ...
  flutter-axi --app driver launch ~/driver-app --device 00920CD6-2DFD-4A6F-9339-709459BBEE60`,
...
examples:
  flutter-axi stopapp
  flutter-axi --app driver stopapp`,
...
examples:
  flutter-axi push --title "New ride" --body "Pickup at Main St"
  ...
  if (payloadFile) {
    ...
    if (!title || !body) {
      throw new FlutterAxiError(
        ...
        "VALIDATION_ERROR",
        ['Run `flutter-axi push --title "New ride" --body "Pickup at Main St"`'],
      );
    }
