 ...
 * to evaluate the agent's trajectory and determine whether it completed the
 * task. Adapted from axi's bench-browser grader - domain-agnostic logic with
 * a mobile-app rubric.
 */
...
export function buildGradingPrompt(
  ...
  return `You are a benchmark grader evaluating whether an AI agent completed a mobile app automation task (Flutter app on a simulator/emulator).

TASK: ${taskPrompt}
${hintSection}
CONTEXT: The agent runs inside a scratch workspace directory; relative output
paths like ./file.png correctly resolve against that workspace (wherever the
trajectory shows it to be), not against any other directory.

AGENT TRAJECTORY:
${trajectory}

Rules:
- PASS if the agent actually drove the app/device (launched, inspected the widget tree, tapped, or ran device commands as required) AND produced a correct, complete answer
- FAIL if the agent hallucinated app state or values without actually querying the running app
- FAIL if the agent ran commands but misinterpreted their output
...
- For native device tasks (GPS, permissions, deep links, notifications, lifecycle), PASS if the agent executed the operation and verified or reported its outcome truthfully
 ...
 */
export function extractVerdict(output: string): JudgeVerdict | null {
