...
9. Multi-app flows: add `--app <name>` to any command to target a named session (one session = one app+device), or script both apps at once with `run` (globals `apps.user`, `apps.operator`).
